...
Build a web-first rhythm game in which a user uploads a music file, the system detects its beats automatically, generates a playable chart, and the user presses one button in time with the music.
...
- A bundled demonstration song that works without an upload.
- User music upload with clear validation, progress, failure, and retry states.
...
- Temporary, private processing of uploaded audio with deletion after analysis by default.
- Progressive Web App support.
- A demonstrated path to iOS/Android through Capacitor.
- Automated tests, deployed end-to-end tests, black-box Product Review, Code Review, and evidence for every delivered story.
...
| API                | Python FastAPI.                                                                                                                      |
| Analysis           | Beat This! for primary beat/downbeat tracking; librosa for onset analysis, baseline comparison, and fallback.                        |
| Data contract      | Versioned chart JSON shared between backend and frontend.                                                                            |
| Frontend hosting   | Static hosting such as Cloudflare Pages. The provider may change without changing the application architecture.                      |
| Processing hosting | Containerized API and analyzer on Cloud Run or an equivalent container platform.                                                     |
| Uploads            | Direct upload to private object storage with a short-lived signed URL.                                                               |
| Jobs               | Asynchronous analysis with persisted status. Long analysis MUST NOT depend on an open browser request.                               |
| Retention          | Delete original uploaded audio immediately after successful or terminally failed analysis; lifecycle deletion is a backstop.         |
...
services/
  api/                    FastAPI control plane and public API
  analyzer/               Audio decoding, analysis, and chart-generation worker
packages/
  chart-schema/           Versioned chart contract and generated clients/types
infra/                    Deployment and environment configuration
tests/
  e2e/                    Black-box deployed Playwright tests
...
- The chart schema is the backend/frontend contract. Framework-specific objects MUST NOT leak across it.
- Beat detection and chart generation are separate. One beat grid can generate several difficulties without re-analysis.
- API request handling and long analysis are separate runtime responsibilities, even if they initially share a container image.
- Provider SDKs stay behind small adapters for storage, queue, database, and hosting.
- The analyzer MUST run locally against a file without the web app.
- Uploaded audio MUST never be committed or included in story evidence.

Expected production flow:

```mermaid
flowchart LR
    A["Web application"] --> B["Create analysis job"]
    B --> C["Short-lived signed upload URL"]
    A --> D["Private temporary object storage"]
    C --> D
    D --> E["Queued analyzer worker"]
    E --> F["Versioned chart JSON"]
    F --> A
    E --> G["Delete uploaded audio"]
    A --> H["Local Web Audio playback"]
```
...
3. Modify or explicitly verify every relevant layer: frontend, backend, shared contract, infrastructure, automated tests, deployed E2E, security/privacy, and product evidence.
4. Deploy an immutable preview of the candidate.
5. Pass automated CI.
...
Large horizontal work such as “build the whole backend,” “add all UI,” or “set up all tests” is prohibited. Infrastructure work MUST be exposed through a small visible outcome, such as a deployed health screen or recoverable analysis job.
...
- Deploys the preview through the normal pipeline.
...
- The deployed preview URL and visible, user-accessible application.
...
The Code Reviewer MUST NOT see or use story screenshots/video, the deployed preview, or Product Review observations before completing the independent assessment.
...
- Security/privacy rules are enforced server-side.
- Tests verify behavior and failure paths without over-mocking internals.
- Dependencies and model licenses are acceptable.
...
The Release Coordinator may be the Delivery Agent after both independent reviews. It verifies CI/reviews/evidence, copies only Code Review status into the product record, creates the final sprint commit, and deploys staging. It MUST NOT change product behavior while finalizing acceptance.
...
5. **Layer coverage** — frontend, backend, contract, infrastructure, CI/CD, E2E, security/privacy.
...
8. **Deployment** — preview/staging identifiers; never credentials.
...
- Come from the deployed preview, never a local dev server.
...
- Be listed in `evidence/manifest.json` with capture time, build identifier, viewport, scenario, and SHA-256 hash.
- Prefer compressed PNG/WebP and WebM. Video larger than 15 MB MUST use Git LFS or be shortened/compressed.

CI SHOULD capture standard evidence automatically. The Delivery Agent may inspect the application while developing, but only the independent Product Reviewer may use evidence to grant functional acceptance. The Code Reviewer MUST not inspect it.
...
- **Delivered** only when status is `deployed`, CI is green, staging smoke tests pass, and the atomic story commit exists.
...
Every acceptance criterion maps to at least one automated test unless it is inherently perceptual. Perceptual criteria require deployed evidence and Product Review.
...
4. **Contract** — frontend types and backend responses conform to chart/OpenAPI schemas.
5. **Backend integration** — real test database, storage/queue adapters, and analyzer boundary where relevant.
6. **Analyzer** — deterministic audio, model invocation, chart invariants, and quality regressions.
7. **Build** — production web and container builds.
8. **Deployed E2E** — Playwright against the immutable preview, not only localhost.
9. **Post-deploy smoke** — staging/production health and one critical path.
10. **Black-box Product Review** — deployed behavior and evidence only.
...
Real-device Product Review MUST cover available desktop Chrome, Firefox, and Safari plus iOS Safari and Android Chrome before public mobile claims. It must exercise keyboard, pointer, touch, built-in output, and at least one Bluetooth path with calibration.
...
- Beat This! and librosa baseline metrics are recorded before the production checkpoint is selected.
...
### 7.5 Upload, privacy, and security tests

Test at minimum:

- Supported formats and boundary sizes/durations.
- Wrong extension/MIME, truncated/corrupt/empty/oversized/excessive media, archives, and non-audio payloads.
- Server-side content validation.
- Signed URL expiry, object isolation, unauthorized job access, and rate limiting.
- Analyzer timeout, crash, retry, duplicate delivery, and idempotency.
- Original deletion after success and terminal failure.
- Lifecycle cleanup of abandoned uploads.
- Logs/errors never expose filenames, object URLs, tokens, or audio.
- One guest cannot access another guest's job/chart.

### 7.6 E2E design

E2E tests MUST:

- Run against the story preview deployment.
...
- Assert user-visible state and backend outcome, not screenshots alone.
...
2. Confirms the preview/build matches the evidence manifest.
3. Executes every acceptance scenario on the deployed preview as a user.
...
story README, its evidence directory, and the deployed preview. Do not inspect
...
- Dependency injection only at real external boundaries: clock, storage, queue, database, and analyzer model.
...
- Reimplementing scoring separately in frontend and backend.
- Spreading provider SDK calls through domain code.
...
screenshots/video, the deployed website, or Product Review observations. Focus
...
## 10. CI/CD and staged deployment

### 10.1 Environments

Use four stages:

1. **Local** — fast development with emulated/test dependencies.
2. **Preview** — immutable per-story candidate; source for E2E and evidence.
3. **Staging** — updated only by accepted story commits; mirrors production with test data.
4. **Production** — promoted only by explicit release stories/checkpoints.

Secrets, buckets, queues, databases, and user data MUST be isolated by environment. Production data MUST NOT be copied into preview/staging.
...
3. Frontend/backend/schema/analyzer tests.
4. Integration tests with test services.
5. Production web/container builds.
6. Migration compatibility checks where applicable.
7. Container vulnerability scan.
8. Immutable preview deployment.
9. Deployed E2E tests.
10. Automated product-evidence capture.
11. Publish inputs for `TEST_EVIDENCE.md`, `DEPLOYMENT.md`, and `manifest.json`.
...
5. Deploy it to staging.
6. Run staging smoke and critical-path E2E.
7. Mark the story `deployed`.

Production promotion MUST use an already-tested immutable build, never rebuild an unpinned branch.

### 10.4 Rollback and flags

Every story identifies a rollback. Uploads, analyzer checkpoints, retention behavior, and chart rules SHOULD have a server-controlled flag/version selector. Database migrations remain backward compatible with the previous application until rollback is no longer needed.
...
**Story:** As a stakeholder, I can open a deployed preview and see that the web app and API are healthy, so every later story has a verified delivery path.

Deliver:

- Repository structure, lockfiles, baseline chart-schema package, and environment conventions.
- Minimal responsive status screen.
- API `/health` and `/version` responses shown by the screen.
- Preview and staging deployments.
- CI static checks, frontend/backend smoke tests, deployed E2E health test, and evidence capture.
...
- API-served demo-song metadata and versioned hand-authored chart.
...
- Chart/song-clock unit tests, API contract test, deployed start-to-finish E2E.
...
- Backend result-summary validation/storage for an anonymous play session without blindly trusting client totals.
- Fake-clock boundary/dropped-frame tests, API validation, and deployed happy/failure E2E.
- Evidence showing calibration, good play, misses, results, and retry.

### Stage B — User music and automatic analysis

#### Sprint 03 — Private audio upload

**Story:** As a player, I can select a supported music file, understand validation errors, and upload it privately for analysis.

Deliver:

- File picker, format/size/duration guidance, progress, cancel, error, and retry.
- API-created guest job and short-lived signed upload.
- Private object storage with random keys and lifecycle cleanup.
- Server-side media validation in an isolated analyzer boundary.
- No permanent audio storage and no real beat model yet; valid files may end as `awaiting_analysis`.
- Integration/deployed E2E for valid, invalid, expired, unauthorized, and oversized uploads.

#### Sprint 04 — Baseline beat grid

**Story:** As a player, I can have an uploaded song analyzed and preview an automatically detected beat grid.

Deliver:

- Worker with FFmpeg normalization and librosa baseline beat/onset analysis.
- Persisted analysis-job state and versioned beat-grid result.
- Progress/polling UI, beat-grid preview, success/failure/retry.
- Audio deletion after terminal processing with cleanup tests.
- Synthetic corpus, beat metrics, analyzer integration/idempotency tests, and deployed E2E with an owned fixture.
...
- Beat This! small/full benchmark against the same corpus.
- Selected checkpoint behind a model interface and version flag.
- Beats, downbeats, confidence signals where available, tempo alternatives, and failure classification.
- Agreement/fallback comparison with librosa.
...
**Story:** As a player, I can choose Easy, Medium, or Hard and play an automatically generated one-button chart for my uploaded song.
...
- Difficulty picker and complete upload-to-play path.
- Invariant/difficulty/golden tests, full deployed E2E, and evidence for all difficulties.
...
- Edit determinism, undo/reset, migration, save/reload, and deployed correction-to-play tests.
- Product Review with deliberately wrong fixture output.

#### Sprint 08 — Leave, return, recover, and retry

**Story:** As a player, I can refresh or leave during processing and safely recover the job or retry a terminal failure.

Deliver:

- Durable queue state machine with idempotent bounded retries.
- Guest capability/token revealing only that guest's job.
- Refresh/reconnect UI, clear retryability, safe cancel, and expiry.
- Worker crash/timeout/duplicate/stale/unauthorized/deletion tests.
- Deployed E2E starting analysis, reloading, recovering, and completing.

#### Sprint 09 — Results and chart-only history

**Story:** As a returning guest, I can see recent charts/results without the service retaining my music.

Deliver:

- Local-first history with chart metadata, score, versions, and retention explanation.
- Backend anonymous result validation and optional chart-only persistence behind the guest capability.
- Re-select-local-file flow when playback audio is absent.
- Clear/delete history behavior.
- Privacy, authorization, retention, migration, and deployed E2E tests.
...
- Responsive upload, editor, game, and results.
...
Uploaded analysis MUST NOT falsely claim to work offline.
...
- Rate limits, quotas, resource limits, upload sandboxing, dependency/container scans, and safe errors.
- Structured job metrics without audio/filename leakage.
- Alerts/dashboards for availability, queue age, analysis duration/failure, and deletion failure.
- Load/soak tests, malformed-media corpus, cost measurement, game-frame profiling, and rollback drill.
- Operational runbook and incident/data-deletion procedure.

#### Sprint 12 — Capacitor mobile build

**Story:** As a mobile tester, I can install the same game as an iOS/Android app and complete demo/upload flows.
...
- Production domain/TLS, CSP/security headers, privacy/retention explanation, upload-rights confirmation, support path, and version display.
- Production smoke/E2E using owned fixtures.
- Backup/restore or reproducibility plan for charts/metadata.
- Rollback/flag drill.
...
- [ ] Frontend, backend, contract, infrastructure, and security effects are implemented or explicitly verified.
- [ ] Unit, component, contract, integration, analyzer, and build tests pass as applicable.
- [ ] The immutable preview is deployed.
...
- [ ] The final commit passes CI and is deployed to staging.
- [ ] Staging smoke tests pass.
- [ ] Story status is `deployed`.

“Works locally,” deferred tests, screenshots without deployed E2E, or one review without the other do not satisfy Done.
...
- Never commit secrets, uploaded/private music, caches, or downloaded models without explicit license approval.
- Large accepted video evidence uses Git LFS.
- Never commit merely to make an incomplete sprint appear delivered.
- After the commit, CI rebuilds/verifies and deploys staging from that exact commit.
...
1. Public maximum upload size/duration. Default proposal: 25 MB and 10 minutes.
2. Mandatory launch devices. Default: current Chrome, Firefox, Safari, iOS Safari, and Android Chrome.
3. Server-side chart persistence versus local history. Default: chart/result metadata may persist behind an anonymous capability; audio never persists.
4. Cloud account/provider and budget. Default architecture: static frontend plus GCP container/storage/queue, CPU first.
